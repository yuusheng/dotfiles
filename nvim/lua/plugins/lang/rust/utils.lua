...
local buf_request = async.wrap(lsp_request, 3)
...
---@param lines table
local function add_macros_lines(lines)
  local err, result = buf_request(0, "rust-analyzer/expandMacro")
...
local function handler(result, ctx)
  ...
  add_macros_lines(markdown_lines)
  hover_float(markdown_lines, result, ctx)
end
...
function M.hover_actions()
  ...
  async.run(function()
    local err, result, context = buf_request(0, "textDocument/hover")
    if err then
      vim.notify("Error fetching hover actions: " .. err, vim.log.levels.ERROR)
      return
    end
